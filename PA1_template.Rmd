...
The median number of steps taken per day is `r medianSteps` and the mean number of steps is `r meanSteps`.
...
Generally speaking, the most steps occur in the `r (averageInterval[averageInterval$steps == max(averageInterval$steps),]$interval)/5`th 5-minute interval, which is the `r averageInterval[averageInterval$steps == max(averageInterval$steps),]$interval`th minute in the day.  Or around `r format(strptime("00:00:00", format = "%H:%M:%S") + duration(hours = 0, minutes = (averageInterval[averageInterval$steps == max(averageInterval$steps),]$interval)), format = "%H:%M:%S")`.
...
It is clear that imputing missing data affects the results of our analysis.  The mean daily steps are `r MeanDay` and the median daily steps are `r MedianDay`.  If you recall, that compares to `r meanSteps` and `r medianSteps`.  Respectively a difference between means of `r MeanDay-meanSteps` and medians of `r MedianDay-medianSteps`.
